  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 16, marginTop: 20 }}>
        <DailyTasksCard data={data} setData={updateData} />
        <SeasonalTrendsCard data={data} setData={updateData} />
        <MessageBoardCard data={data} setData={updateData} currentUser={currentUser} />
      </div>
...
function LabelPrintingTab({ data, setData, mode }) {
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: mode === "mobile" ? "1fr" : "repeat(auto-fill, minmax(280px,1fr))", gap: 12 }}>
        {items.map((item) => <LabelCard key={item.id} item={item} setData={updateData} />)}
...
function App() {
  ...
  const lastSavedMutationRef = useRef(0);
  const legacyInventoryRef = useRef(null);
  const migrationDoneRef = useRef(false);
  const remoteSnapshotMutationRef = useRef(0);
   useEffect(() => {
    ...
    const unsubscribeAuth = auth.onAuthStateChanged((user) => {
      ...
      legacyInventoryRef.current = null;
      remoteUpdateRef.current = true;
      remoteSnapshotMutationRef.current = localMutationRef.current;
      ...
      unsubscribeShared = sharedDocRef.onSnapshot(
        (snap) => {
          ...
          if (snap.exists) {
            ...
            // Legacy inventory migration is completed BEFORE hydration is marked
            // ready. This prevents an empty inventory snapshot from ever being
            // mistaken for the real inventory and written back to Firestore.
            legacyInventoryRef.current = Array.isArray(legacyInventory) ? legacyInventory : null;
            if (!legacyInventory || legacyInventory.length === 0) {
              migrationDoneRef.current = true;
            } else if (!migrationDoneRef.current) {
              inventoryColRef.get().then(async (existing) => {
                if (!existing.empty) {
                  migrationDoneRef.current = true;
                  return;
                }
                const items = legacyInventory.filter((item) => item && item.id);
      ...
      unsubscribeInventory = inventoryColRef.onSnapshot(
        (snap) => {
          ...
          // If a legacy inventory exists, the migration must populate the
          // collection before this becomes the authoritative snapshot.
          if (inventory.length === 0 && Array.isArray(legacyInventoryRef.current) && legacyInventoryRef.current.length > 0) return;
  ...
  useEffect(() => {
    ...
    // Firestore listeners update state during startup. Those renders must never
    // trigger a save. Only a mutation made through updateData() is persisted.
    if (remoteUpdateRef.current && localMutationRef.current === remoteSnapshotMutationRef.current) {
      remoteUpdateRef.current = false;
      lastSavedMutationRef.current = localMutationRef.current;
      return;
    }
    if (localMutationRef.current <= lastSavedMutationRef.current) return;

    if (writeTimerRef.current) clearTimeout(writeTimerRef.current);
    const mutationAtSchedule = localMutationRef.current;
    ...
    writeTimerRef.current = setTimeout(async () => {
      try {
        ...
        const operations = [];
        (data.inventory || []).forEach((item) => {
          if (item && item.id) operations.push({ type: "set", id: String(item.id), item });
        });
        oldIds.forEach((id) => {
        ...
        lastSavedMutationRef.current = mutationAtSchedule;
      } catch (e) {
  ...
  // All UI writes go through this wrapper so remote Firestore snapshots can
  // never be confused with a user change.
        ...
        {!currentUser ? (
          ...
          <div style={{
            ...
            <div style={{
              ...
              {active === "overview" && <OverviewTab data={data} setData={updateData} currentUser={currentUser} />}
              {active === "pnl" && <PnLTab data={data} mode={mode} />}
              {active === "expenses" && <ExpensesTab data={data} setData={updateData} mode={mode} />}
              {active === "balepnl" && <BalePnLTab data={data} setData={updateData} mode={mode} />}
              {active === "payout" && <PayoutTab data={data} setData={updateData} mode={mode} />}
              {active === "ai" && <AIDescriptionsTab mode={mode} />}
              {active === "scanner" && <PriceScannerTab mode={mode} />}
              {active === "inventory" && <InventoryTab data={data} setData={updateData} mode={mode} />}
              {active === "sold" && <SoldItemsTab data={data} setData={updateData} mode={mode} />}
              {active === "labels" && <LabelPrintingTab data={data} setData={updateData} mode={mode} />}
              {active === "templates" && <MessagingTemplatesTab data={data} setData={updateData} mode={mode} />}
              {active === "archive" && <ArchiveTab data={data} setData={updateData} mode={mode} />}
              {active === "auth" && <AuthHelpTab />}
              {active === "settings" && <SettingsTab data={data} setData={updateData} currentUser={currentUser} mode={mode} />}
            </div>
          </div>
        )}
      </div>
