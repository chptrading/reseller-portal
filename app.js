...
// Persist Firestore's local cache to IndexedDB. Once a write is handed to
// the SDK it's durable on this device immediately and will sync to the
// server in the background — even if the tab is refreshed or closed before
// the network request finishes. Without this, an in-flight write that
// hasn't reached the server yet is simply lost on refresh.
...
// Inventory items are written straight to their own Firestore document the
// moment a user saves/edits/archives/deletes one — not batched into the
// generic debounced full-array sync below. That whole-array sync diffs the
// *locally cached* inventory against Firestore, so if two people (or two
// tabs) edit different items around the same time, whichever save fires
// last would silently overwrite the other person's change with its own
// stale copy of items it didn't touch. Writing directly by item id avoids
// that: a save can only ever affect the one document it names.
function writeInventoryItem(item) {
  if (!item || item.id == null) return;
  inventoryColRef.doc(String(item.id)).set(item).catch((e) => console.error("Inventory item write failed:", e));
}
function deleteInventoryItem(id) {
  if (id == null) return;
  inventoryColRef.doc(String(id)).delete().catch((e) => console.error("Inventory item delete failed:", e));
}
...
Items (photos follow in the same order):\n`;
        ...
        setError(e && e.message ? e.message : "Something went wrong generating the batch listing. Try again with fewer items, or check your Gemini API key.");
      }
      setLoading(false);
    })();
  }, []);
...
function InventoryTab({ data, setData, mode }) {
  ...
  const applyAutoList = ({ description, price }) => {
    ...
    setData((d) => ({
      ...d,
      inventory: d.inventory.map((i) => {
        if (i.id !== autoListItem.id) return i;
        saved = {
          ...i,
          description,
    ...
    if (saved) writeInventoryItem(saved);
  ...
  const save = () => {
    ...
    setData((d) => ({
      ...
      inventory: d.inventory.map((i) => {
        ...
        saved_ = { ...i, trackingNumber: tracking, labelPhoto: labelPhoto[0] || null, labelPrinted: !!(labelPhoto[0] || tracking) };
        return saved_;
      }),
    }));
    if (saved_) writeInventoryItem(saved_);
    setSaved(true);
...
// ---------- Error boundary ----------
// Without this, any error thrown while rendering the logged-in dashboard
// (a bad Firestore document shape, an undefined field, etc.) causes React
// to silently unmount the whole tree, which looks exactly like a white
// screen with nothing in the console explaining why. This catches that
// and shows the real error instead, plus a way to recover.
class ErrorBoundary extends React.Component {
  ...
  componentDidCatch(error, info) {
    console.error("Reseller Portal crashed:", error, info && info.componentStack);
  }
  render() {
    if (this.state.error) {
      ...
      return (
        <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: 24, fontFamily: "Arial, sans-serif" }}>
          <div style={{ maxWidth: 700 }}>
            <h2 style={{ color: "#C23B3B" }}>Something went wrong</h2>
            <p style={{ color: "#5B7192" }}>The app hit an error while rendering. Screenshot this and send it over — it tells us exactly what to fix.</p>
            <pre style={{ whiteSpace: "pre-wrap", background: "#F4F7FC", padding: 14, borderRadius: 8, fontSize: 12.5, overflow: "auto", maxHeight: 300 }}>{message}</pre>
            <button
              onClick={() => window.location.reload()}
              style={{ marginTop: 12, background: "#1B5FE0", color: "#fff", border: "none", borderRadius: 8, padding: "9px 16px", fontSize: 14, fontWeight: 600, cursor: "pointer" }}
            >
              Reload
            </button>
          </div>
...
function App() {
  ...
  useEffect(() => {
    ...
    const flush = async () => {
      try {
        // Inventory items are written straight to their own document by
        // writeInventoryItem/deleteInventoryItem the moment they're
        // edited (see above) — NOT reconciled here. This block used to
        // diff the whole locally-cached inventory array against Firestore
        // and re-set every item on every change. That meant: if your copy
        // of the inventory hadn't yet received someone else's (or another
        // tab's) recent edit when you saved something unrelated, this
        // would re-write their item too, using your stale copy, silently
        // reverting their change. Only the small, non-inventory shared
        // document (settings, expenses, messages, etc.) is synced here.
    ...
    // Short debounce so rapid changes to the shared doc (e.g. toggling
    // dark mode, adding an expense) don't fire a write per keystroke,
    // capped well below a second so nothing sits unsaved for long.
    // pendingFlushRef always points at the latest flush so a page-hide can
    // trigger it immediately regardless of where the debounce timer is.
    pendingFlushRef.current = flush;
    writeTimerRef.current = setTimeout(flush, 150);
  ...
  // Flush any pending write immediately when the tab is hidden, closed, or
  // refreshed, instead of waiting on the debounce timer.
  useEffect(() => {
    const flushNow = () => {
      if (writeTimerRef.current) {
        clearTimeout(writeTimerRef.current);
