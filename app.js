...
// Get this from https://aistudio.google.com/apikey (free)
const GEMINI_API_KEY = "AQ.Ab8RN6J6Msls9hbIZuWobJ0oUDFGZQEsAk9FtJF3szVSWhXKVw";
...
async function callGemini(promptText, images = [], jsonMode = false) {
  ...
  if (!cand || !cand.content) throw new Error("No response from AI");
...
function App() {
      ...
      unsubscribeFirestore = sharedDocRef.onSnapshot(
        (snap) => {
          if (snap.exists()) {
            ...
            sharedDocRef.set(DEFAULT_DATA).catch((e) => console.error("Initial Firestore write failed:", e));
          }
          setLoading(false);
        },
        (err) => {
  ...
  useEffect(() => {
    ...
    writeTimerRef.current = setTimeout(() => {
      // Never write the legacy users/password field back to Firestore.
      const { users: _legacyUsers, ...safeData } = data;
      sharedDocRef.set(safeData).catch((e) => console.error("Firestore write failed:", e));
    }, 500);
        ...
        {!currentUser ? (
          ...
          <div style={{
            ...
            <div style={{
              ...
              padding: mode === "mobile" ? "16px 14px" : "26px 30px",
              paddingTop: mode === "mobile" ? `max(16px, ${SAFE_TOP})` : `max(26px, ${SAFE_TOP})`,
              paddingBottom: mode === "mobile" ? `max(16px, ${SAFE_BOTTOM})` : `max(26px, ${SAFE_BOTTOM})`,
            }}>
